...
export function validateParentSasaranExists(result, raw, options = {}) {
  ...
  if (!key) {
    addValidationError(result, "PARENT_SASARAN_KEY_REQUIRED", "sasaran_unique_key", "sasaran_unique_key wajib diisi untuk pendampingan.", {});
    return result;
  }
  ...
  if (!registry.has(key)) {
    addValidationError(result, "PARENT_SASARAN_NOT_FOUND", "sasaran_unique_key", "Parent sasaran tidak ditemukan pada registry validasi.", { sasaran_unique_key: key });
  }
